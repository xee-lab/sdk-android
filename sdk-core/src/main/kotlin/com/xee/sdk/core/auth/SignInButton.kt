...
/**
 * SignIn button to authenticate the user
...
class SignInButton : Button {
    ...
    private fun init(context: Context, attrs: AttributeSet?, defStyle: Int) {
        val a = getContext().obtainStyledAttributes(attrs, R.styleable.SignInButton, defStyle, 0)
        theme = a.getInt(R.styleable.SignInButton_signInBtnTheme, Theme.GREY.ordinal)
        size = a.getInt(R.styleable.SignInButton_signInBtnSize, sizeNormal)
        ...
        paddingHorizontal = getRealDimen(R.dimen.sign_in_padding_horizontal)
        paddingVertical = getRealDimen(R.dimen.sign_in_padding_vertical)
        paddingDrawable = getRealDimen(R.dimen.sign_in_padding_drawable)
    ...
    private fun setBtnSize(context: Context, size: Int) {
        ...
        if (size == sizeIcon) {
            ...
            text = context.getString(R.string.sign_in_with_xee)
            setBtnPadding(paddingHorizontal, paddingVertical, paddingHorizontal, paddingVertical)
            height = minHeight
     ...
     */
    fun setOnSignInClickResult(xeeAuth: XeeAuth, connectionCallback: AuthenticationCallback) {
        authenticationCallback = connectionCallback
     ...
     */
    private fun signIn() {
        xeeWeakReference?.get()?.connect(object : AuthenticationCallback {
